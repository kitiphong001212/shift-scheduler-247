...
const form = reactive<{ name: string; active: boolean }>({
  name: '', active: true
})

function openCreate() {
  editingId.value = null
  Object.assign(form, { name: '', active: true })
  error.value = ''
  showForm.value = true
}
function openEdit(e: Employee) {
  editingId.value = e.id
  Object.assign(form, { name: e.name, active: e.active })
  error.value = ''
  showForm.value = true
}
function submit() {
  ...
  try {
    ...
    else store.addEmployee({ code: '', name: form.name.trim(), defaultShift: 'A1', active: form.active })
